...
const ScrollingBanner = () => {
  ...
  return (
    <div className="relative overflow-hidden bg-gradient-to-r from-white via-[#f9fbff] to-white py-8">
      {/* Scrolling Text */}
      <motion.div
        className="whitespace-nowrap text-[6rem] font-extrabold text-[#008300] tracking-[0.15em] leading-tight"
        animate={{ x: ["0%", "-100%"] }}
        transition={{
          ease: "linear",
          duration: 25,
            ...
            {text.split("*").map((part, idx) => (
              <React.Fragment key={idx}>
                ...
                {idx < text.split("*").length - 1 && (
                  <motion.span
                    className="inline-block mx-12 text-orange-500 text-[6.5rem] font-bold"
                    whileHover={{
                      rotate: 360,
                      scale: 1.3,
      ...
      <div className="fixed right-8 bottom-20 flex flex-col gap-4 z-50">
        {[
          ...
          <motion.div
            key={index}
            className={`${item.bg} p-4 rounded-full text-orange-500 shadow-md cursor-pointer`}
            whileHover={{
              scale: 1.25,
              rotate: 360,
